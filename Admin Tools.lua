...
function main()
  ...
  wait(1000)
    sampAddChatMessage("{408ad8}[Admin Tools]: {FFFFFF}Автор данного скрипта {408ad8}Alonso_Whittaker. {FFFFFF}Версия: {408ad8}0.2.", -1)
    ...
    downloadUrlToFile(update_url, update_path, function(id, status)
        if status == dlstatus.STATUS_ENDDOWNLOADDATA then
            ...
            if tonumber(updateIni.update.vers) > script_vers then
                sampAddChatMessage("{408ad8}[Admin Tools]: {FFFFFF}Разработчик выпустил обновление. Версия:{408ad8} " .. updateIni.update.vers_text, -1)
                update_state = true
            end
    ...
    wait(0)
        ...
        if update_state then
            downloadUrlToFile(script_url, script_path, function(id, status)
                if status == dlstatus.STATUS_ENDDOWNLOADDATA then
                    sampAddChatMessage("{408ad8}Admin Tools был успешно обновлен.", -1)
                    thisScript():reload()
                end
            end)
            break
...
---------------------------------------------------------------
  if ban_menu.v then
    imgui.SetNextWindowSize(imgui.ImVec2(350, 310), imgui.Cond.FirstUseEver)
    imgui.ShowCursor = true
...
---------------------------------------------------------------
  if p_menu.v then
    ...
    imgui.Begin(u8"Меню быстрого ответа | Alonso_Whittaker", p_menu, imgui.WindowFlags.NoMove + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize)
        ...
        sampSendChat("/pm " .. banId .. " Игрок, по Вашей жалобе был наказан. Спасибо за Ваш репорт.")
            end
...
---------------------------------------------------------------
  if nakm_menu.v then
    imgui.SetNextWindowSize(imgui.ImVec2(350, 295), imgui.Cond.FirstUseEver)
    imgui.ShowCursor = true
    imgui.SetNextWindowPos(imgui.ImVec2((sw / 2), sh / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
